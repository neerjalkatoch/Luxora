...
for fname in service_pages:
    ...
    # Remove old schemas just in case
    content = re.sub(r'<script type="application/ld\+json">.*?</script>\s*', '', content, flags=re.DOTALL|re.IGNORECASE)

    content = content.replace('</head>', service_schema + '</head>')
